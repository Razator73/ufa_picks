...
class TestGameCoverage:
    def test_team_methods(self, db):
        ...
        game1 = Game(id="G1", home_team_id=team1.id, away_team_id=team2.id, status="Final", home_score=10, away_score=5, season="2025", week=1, streaming_url="", has_roster_report=False)
        game2 = Game(id="G2", home_team_id=team2.id, away_team_id=team1.id, status="Final", home_score=5, away_score=10, season="2025", week=2, streaming_url="", has_roster_report=False)
        game3 = Game(id="G3", home_team_id=team1.id, away_team_id=team2.id, status="Final", home_score=5, away_score=10, season="2025", week=3, streaming_url="", has_roster_report=False)
    ...
    def test_game_methods(self):
        game = Game(id="G4", status="Scheduled", week=4, streaming_url="", has_roster_report=False, home_score=10, away_score=5)
        assert game.winner is None

        game2 = Game(id="G5", status="Scheduled", week=5, streaming_url="", has_roster_report=False, home_score=None, away_score=None)
...
class TestUserCoverage:
    ...
    def test_pick_methods(self, db, user):
        ...
        game = Game(id="G6", home_team_id=team1.id, away_team_id=team2.id, status="Scheduled", home_score=10, away_score=5, season="2025", week=1, streaming_url="", has_roster_report=False)
        db.session.add(game)
        
        pick = Pick(user_id=user.id, game_id=game.id, home_team_score=10, away_team_score=5)
        db.session.add(pick)
        db.session.commit()
